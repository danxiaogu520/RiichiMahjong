  ...
  appRoot.innerHTML = `
    <section class="table-shell">
      ...
      <div class="mahjong-table">
        <div class="table-center"><strong>${gameState ? `${windName(gameState.wind)}${gameState.round}局` : "等待牌局"}</strong><span>${gameState ? `剩余 ${gameState.remaining_tiles} 张 · ${phaseName(gameState.phase)}` : "等待服务器状态…"}</span></div>
...
function renderSeat(index: number, name: string, ownIndex: number): string {
  const active = phasePlayer() === index ? " active-seat" : "";
...
function renderActions(): string {
  if (callRequest?.player === playerIndex(session!.player)) return callRequest.options.map((option, index) => Object.entries(option.call_type).map(([kind, payload]) => `<button data-action="call:${kind}:${index}">${callName(kind)}${callTiles(payload)}</button>`).join("")).join("") + `<button class="secondary-button" data-action="call:Pass:-1">跳过</button>`;
  if (actionRequest?.player !== playerIndex(session!.player)) return "";
...
function updateCountdown(generation: number): void {
  ...
  if (generation !== countdownGeneration || !countdown || !actionDeadline) return;
...
function tileTypeLabel(type: number): string { return tileLabel(type * 4); }
