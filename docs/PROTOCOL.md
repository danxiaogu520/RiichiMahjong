...
分析信息包括牌效、听牌和弃牌建议，属于辅助展示数据，不改变对局结果。`GameStateView.analysis` 由服务端按收件人手牌计算后随状态更新下发；`RoundResult.win_details` 包含本局和牌役种、符数与番数明细（按座位带前缀，多人荣和时包含所有和了者），流局时为空。服务端和客户端应允许未来增加事件类型；未知事件不能导致已完成对局被错误回滚。
