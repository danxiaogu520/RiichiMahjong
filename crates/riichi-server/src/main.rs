...
use riichi_engine::action::{CallType, GameEvent, RoundEndReason, ResponseAction, TurnAction};
use riichi_engine::game::{GamePhase, GameState};
use riichi_engine::player::wind_display;
...
fn ctt(tt: TileType) -> String {
    let code = match tt.suit() {
        riichi_core::tile::Suit::Sou => "\x1b[31m",   // 索子 → 红
        riichi_core::tile::Suit::Pin => "\x1b[32m",   // 筒子 → 绿
        riichi_core::tile::Suit::Man => "\x1b[36m",   // 万子 → 青
        riichi_core::tile::Suit::Wind | riichi_core::tile::Suit::Dragon => "\x1b[33m", // 字牌 → 黄
    };
...
/// 选择立直宣言牌（必须是打出后能听牌的牌）
fn get_riichi_discard_choice(game: &GameState) -> riichi_core::tile::Tile {
...
/// 选择打出哪张牌
fn get_discard_choice(game: &GameState) -> riichi_core::tile::Tile {
    let me = &game.players[0];
    let forbidden = &me.forbidden;
...
fn display_state(game: &GameState) {
    ...
    if !me.is_riichi && me.is_menzen() {
        ...
        let waits = riichi_logic::analysis::analyze_wait_tiles(me.hand.tiles());
        if !waits.is_empty() {
...
/// 显示对手牌河（每行 6 张，带边框）
fn print_discards_in_grid(p: &riichi_engine::player::Player, prefix: &str) {
...
/// 显示自己牌河（无边框，扁平）
fn print_discards_flat(p: &riichi_engine::player::Player) {
    if p.discards.is_empty() {
        println!();
        return;
    }
...
/// 显示副露选项
fn display_call_options(options: &[&riichi_engine::action::CallOption]) {
    print!("  > ");
...
/// 获取副露选择
fn get_call_choice(options: &[&riichi_engine::action::CallOption]) -> Option<ResponseAction> {
    loop {
        print!("  选择: ");
        io::stdout().flush().unwrap();
