...
pub enum GamePhase {
    /// 摸牌阶段：当前玩家从牌山摸一张牌
    DrawPhase,
    /// 行动阶段：摸牌后，玩家选择 自摸/立直/暗杠/加杠/打牌
    ActionPhase,
    /// 响应阶段：其他玩家对打出的牌做出响应（吃/碰/杠/荣和）
    ResponsePhase {
        discarded_tile: Tile,
        discarder: PlayerId,
    },
    /// 抢杠荣和响应阶段：加杠后，其他人仅可荣和（不可吃/碰/杠）
    ChankanResponse {
        kakan_tile: Tile,
...
/// 从副露中提取食替禁打的牌类型
///
/// 碰/吃后，手牌贡献的牌不能立刻打出（按类型判断）。
/// - 碰：与被碰牌同类型的牌
/// - 吃：手牌中参与吃的两张牌的类型
fn extract_kuikae_tiles(meld: &Meld) -> Vec<TileType> {
...
impl GameState {
    ...
    fn reveal_dora_indicator(&mut self) {
        ...
        // 宝牌指示牌位置：131, 130, 129, 128, 127（最多 5 张）
        let idx = DORA_INDICATOR.saturating_sub(kan_count - 1);
        if idx >= 127 && self.dora.len() < 5 && self.wall.len() > idx {
            let indicator = self.wall[idx].tile_type();
            self.dora_indicators.push(indicator);
            self.dora.push(Self::dora_from_indicator(indicator));
            // 里宝牌指示牌
            let ura_idx = idx.saturating_sub(5);
            if ura_idx < self.wall.len() {
                self.ura_dora_indicators.push(self.wall[ura_idx].tile_type());
            }
        }
    }
}
...
impl GameState {
    ...
    pub fn start_round(&mut self, rng: &mut impl Rng) {
        self.wall = Tile::all_tiles();
        self.wall.shuffle(rng);
        self.current_index = 0;
        self.drawn_tile = None;
        self.dora.clear();
        self.dora_indicators.clear();
        self.ura_dora_indicators.clear();
        // 翻第一张宝牌指示牌（wall[DORA_INDICATOR]）
        let indicator = self.wall[DORA_INDICATOR].tile_type();
        self.dora_indicators.push(indicator);
        self.dora.push(Self::dora_from_indicator(indicator));
        // 里宝牌指示牌在宝牌指示牌的下方（wall[DORA_INDICATOR - 5]）
        let ura_idx = DORA_INDICATOR.saturating_sub(5);
        if ura_idx < self.wall.len() {
            self.ura_dora_indicators.push(self.wall[ura_idx].tile_type());
        }
    ...
    pub fn get_kakan_options(&self, player: PlayerId) -> Vec<(usize, Tile)> {
        ...
        for (i, meld) in p.melds.iter().enumerate() {
            if meld.kind == mahjong_core::meld::MeldKind::Pon {
    ...
    pub fn execute_kakan(
        ...
        let original_pon;
        {
            ...
            new_tiles.push(tile);
            *meld = Meld {
                kind: mahjong_core::meld::MeldKind::Kakan,
                tiles: new_tiles,
                called_tile,
                from_player,
            };
...
impl GameState {
    ...
    pub fn is_rinshan_tile(&self, tile: Tile) -> bool {
        self.wall.len() > 135 && (132..=135).any(|i| self.wall[i] == tile)
    }
