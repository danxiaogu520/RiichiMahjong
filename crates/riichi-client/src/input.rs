...
use riichi_engine::action::ResponseAction;
...
pub fn handle_input(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Char('t') => {
            if app.game.check_tsumo(riichi_core::player::PlayerId(0)).is_some() {
                app.execute_tsumo();
            } else {
        ...
        KeyCode::Char('r') => {
            if app.game.can_declare_riichi(riichi_core::player::PlayerId(0)) {
                app.execute_riichi();
            } else {
        ...
        KeyCode::Enter => {
            if app.call_selected < option_count {
                ...
                let action = match &opt.call_type {
                    riichi_engine::action::CallType::Ron => ResponseAction::Ron,
                    riichi_engine::action::CallType::Pon { hand_tiles } => {
                        ResponseAction::Pon { hand_tiles: *hand_tiles }
                    }
                    riichi_engine::action::CallType::Chi { hand_tiles } => {
                        ResponseAction::Chi { hand_tiles: *hand_tiles }
                    }
                    riichi_engine::action::CallType::Minkan { hand_tiles } => {
