...
use riichi_engine::action::CallType;
...
fn render_action_line(app: &App) -> Line<'static> {
    ...
    let can_tsumo = app.game.check_tsumo(riichi_core::player::PlayerId(0)).is_some();
    let can_riichi = app.game.can_declare_riichi(riichi_core::player::PlayerId(0));
